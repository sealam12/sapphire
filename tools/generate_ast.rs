...
fn define_ast(output_directory: String, base_name: String, imports: String, types: Vec<String>) -> io::Result<()> {
    println!("---- Generating AST for {} ----", base_name);
    ...
    println!("---- Finished AST for {}   ----", base_name);
...
fn main() {
    ...
    define_ast(output_dir.clone(), "Expr".to_owned(), 
        ...
        vec![
            String::from("Call       :Box<Expr> callee, Token paren, Vec<Expr> arguments"),
            String::from("Index      :Box<Expr> indexee, Token bracket, Box<Expr> index"),
            ...
            String::from("Logical    :Box<Expr> left, Token operator, Box<Expr> right"),
